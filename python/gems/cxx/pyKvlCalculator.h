...
public:
    ...
    KvlCostAndGradientCalculator(std::string typeName,                                  // not passed, only used here , 
                                 std::vector<KvlImage> images,
                                 std::string boundaryCondition,                         // not passed to c part
        ...
        // add if/else blocks here for each of the new calcs (DSWbeta, Frobenius, Wishart)
        // are these created right? they should all be in the log domain, yes?
        // if they are all in the log domain, then we can drop these
    ...
    // one set params for each DTI calculator
